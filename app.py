...
def get_ai_suggestion(amt, stock_info):
    prompt = "You are Indian stock market advisor. Reply in Marathi only. I have Rs " + str(amt) + " to invest today. Market data:\n" + stock_info + "\nGive specific stock buy suggestion with amount allocation and reason based on RSI. Also suggest paper trade F&O opportunity for learning."
...
if st.button("Market Analyze करा"):
    ...
    with st.spinner("AI विचार करतोय..."):
        suggestion = get_ai_suggestion(amt, stock_info)
    st.write(suggestion)
